...
import br.com.laserviewpr.conversor.ConvertPDF;
import br.com.laserviewpr.infra.DAO.DaoFactory;
import br.com.laserviewpr.infra.report.GenericReport;
import br.com.laserviewpr.infra.report.ReportParameters;
import br.com.laserviewpr.infra.report.laudo.LaudoReport;
...
import br.com.laserviewpr.merge.pdf.MergePDF;
...

{
	    ...
	    ServletException
	{
		...
		catch (Exception ex)
		{
			...
			res.getOutputStream().println(
			    "<html><p><span style='font-size:12pt; color:#0000FF;'>Ocorreu um erro ao Gerar o Laudo, favor entrar em contato com a Laser View.</span></p>");
			res.getOutputStream().println(
	    ...
	    throws Exception
	{
		...
		if ("rtf".equals(str))
		{
			...
			// imagens
			// TODO: APENAS TESTE DE LEITURA DA TABELA DE TESTE DA Laser View E
			// ESCREVER EM ARQUIVO
	...
	private final String assinaturaDigitalMedico (final Blob blob, String nomeMedico)
	{
		...
		{
			nomeMedico = "assinaturaMedicoLaserView";
		}
		try
		{
	...
	private void testeLerFotoGravarEmArquivo () throws Exception
	{
		try
		{
			...
			{
				final String output = "c://temp//img_laserView//RETORNO_TABELA_LIXOIMAGEM_ID_"
