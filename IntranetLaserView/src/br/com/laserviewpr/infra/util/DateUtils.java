package br.com.laserviewpr.infra.util;
...
public final class DateUtils implements Serializable {
	...
	protected static final TimeZone TIME_ZONE = TimeZone
			.getTimeZone(Constants.Default.timeZone);
	protected static final SimpleDateFormat simpleDateFormat = new SimpleDateFormat(
			Constants.Default.formatDate);
	protected static final SimpleDateFormat simpleDateMinFormat = new SimpleDateFormat(
			Constants.Default.formatDateMin);
	 ...
	 * @return
	 */
	public static java.util.Date addAmountDate(java.util.Date dateTime,
			int amount, int field) {
		Calendar calendar = Calendar.getInstance();
	...
	public static final String firstDayMonth() {
		...
		return "01/" + (month < 10 ? ("0" + month) : month) + "/"
				+ DateUtils.getYear();
	}
	...
	/**
	 * converte uma data string no formato ddmmyy (ex: 010112) para 01/01/12
	 * 
	 * @param minDate
	 * @return String
	 */
	public static String convertToMinDateDDMMYY(String minDate) {
		if (minDate != null && minDate.length() == 6) {
			String dia = minDate.substring(0, 2);
			String mes = minDate.substring(2, 4);
			String ano = minDate.substring(4, 6);
			return (dia + "/" + mes + "/" + ano);
		}
		return null;
	}
	...
	public static String formatDate(java.util.Date dateTime) {
		...
		try {
			if (dateTime != null) {
				...
				if (dateTimeFormat.substring(11, 19).equalsIgnoreCase(
						"00:00:00")) {
	...
	public static java.util.Date parseDate(String date) {
		...
		try {
			if (!date.contains("/") && (date.length() == 8)) {
				...
				date = date.substring(0, 2).concat("/").concat(
						date.substring(2, 4).concat("/")).concat(
						date.substring(4, 8));
			} else if (date.contains("/")
					&& ((date.length() + 2) == Constants.Default.formatDate
							.length())) {
			...
			} else if (date.length() == Constants.Default.formatDateTime
					.length()) {
				simpleDateTimeFormat.setTimeZone(TIME_ZONE);
			...
			} else if (date.length() == Constants.Default.formatDateMin
					.length()) {
				simpleDateMinFormat.setTimeZone(TIME_ZONE);
				dt = simpleDateMinFormat.parse(date);
			}
			return dt;
	...
	public static java.util.Date parseTime(String time) {
		java.util.Date dt = null;
		try {
			if ((time.length() + 3) == Constants.Default.formatTimeHHMMSS
					.length()) {
				time += ":00";
			}
	 ...
	 */
	public static final String validaIntervalo(String dataIni, String dataFim,
			byte typeInterval, Integer interval) {
		try {
			if ((dataIni != null)
					&& (dataFim != null)
					&& (dataIni.length() == Constants.Default.formatDate
							.length())
					&& (dataFim.length() == Constants.Default.formatDate
							.length())) {
				Calendar calendar = Calendar.getInstance();
				calendar.setTimeZone(TIME_ZONE);
				...
				default: {
					...
					Integer intervaloMax = (Integer) context
							.lookup("java:comp/env/maxInterval");
					context = null;
	 ...
	 * @return int
	 */
	public static int getNumberDays(java.util.Date initDate,
	 ...
	 * @return String
	 */
	public static final String getNumberDaysHours(java.util.Date initDate,
			java.util.Date endDate) {
		if ((initDate == null) || (endDate == null)) {
			return null;
		}
	 ...
	 * @return
	 */
	public static boolean isSameMonth(java.util.Date initialDate,
			java.util.Date finalDate) {
		return (isSameTime(initialDate, finalDate, Calendar.MONTH) && isSameYear(
	 ...
	 * @return
	 */
	public static boolean isSameYear(java.util.Date initialDate,
			java.util.Date finalDate) {
	...
	private static final boolean isSameTime(final java.util.Date initialDate,
		...
		case Calendar.WEEK_OF_MONTH:
			return (getCalendar(initialDate).get(Calendar.WEEK_OF_MONTH) == getCalendar(
					finalDate).get(Calendar.WEEK_OF_MONTH));
		case Calendar.MONTH:
			return (getCalendar(initialDate).get(Calendar.MONTH) == getCalendar(
					finalDate).get(Calendar.MONTH));
		case Calendar.YEAR:
			return (getCalendar(initialDate).get(Calendar.YEAR) == getCalendar(
	 ...
	 * 
	 * @param date
	 *            a partir da qual será localizado o próximo dia útil.
	 * @return Date Dia útil.
	 */
	public static final java.util.Date proximoDiaUtil(Timestamp date)
			throws Exception {
		if (date == null) {
			throw new Exception(
